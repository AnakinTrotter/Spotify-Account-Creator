...
headers = {'User-agent': 'S4A/2.0.15 (com.spotify.s4a; build:201500080; iOS 13.4.0) Alamofire/4.9.0',
           'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8',
           'Accept': 'application/json, text/plain;q=0.2, */*;q=0.1', 'App-Platform': 'IOS', 'Spotify-App': 'S4A',
           'Accept-Language': 'en-TZ;q=1.0', 'Accept-Encoding': 'gzip;q=1.0, compress;q=0.5',
           'Spotify-App-Version': '2.0.15'}
domains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'hotmail.co.uk', 'hotmail.fr', 'outlook.com', 'icloud.com',
           'mail.com', 'live.com', 'yahoo.it', 'yahoo.ca', 'yahoo.in', 'live.se', 'orange.fr', 'msn.com', 'mail.ru',
           'mac.com']
lock = Lock()
...
class AccountGenerator:
    ...
    def run(self, proxy_type='http', account_num=5):
        ...
        self.amount = account_num

        print("\nPlease be patient with the free proxies.")
    ...
    def update_title(self):
        if self.variables['unlimited']:
            while True:
                ...
                system(
                    'title [Spotify Account Creator] - Created: %s ^| Retries: %s ^| CPM: %s ^| Time Elapsed: %s ^| '
                    'Threads: %s' % (
                        self.variables['created'], self.variables['retries'], self.variables['cpm'], elapsed,
                        (active_count() - 2)))
                sleep(0.4)
        else:
            while self.variables['created'] != int(self.amount):
                elapsed = strftime('%H:%M:%S', gmtime(time() - self.start))
                system(
                    'title [Spotify Account Creator] - Created: %s/%s ^| Retries: %s ^| CPM: %s ^| Time Elapsed: %s '
                    '^| Threads: %s' % (
                        self.variables['created'], self.amount, self.variables['retries'], self.variables['cpm'],
                        elapsed,
                        (active_count() - 2)))
                sleep(0.4)
            ...
            system(
                'title [Spotify Account Creator] - Created: %s/%s ^| Retries: %s ^| CPM: %s ^| Time Elapsed: %s ^| '
                'Threads: %s' % (
                    self.variables['created'], self.amount, self.variables['retries'], self.variables['cpm'], elapsed,
                    (active_count() - 2)))
    ...
    def creator(self, proxy):
        ...
        try:
            create = post('https://spclient.wg.spotify.com/signup/public/v1/account', data=data, headers=headers,
                          proxies={'https': '%s://%s' % (self.proxy_type, proxy)}, timeout=5)
            if create.json()['status'] == 1:
                username = create.json()['username']
                if username != '':
                    self.users.append(username)
                    self.passes.append(password)
                    self.write('%s[%sCREATED%s] %s:%s | Username: %s | Gender: %s | Date of Birth: %s/%s-%s' % (
                    ...
                    with open('Created [RAW].txt', 'a', encoding='UTF-8') as f:
                        f.write('%s:%s\n' % (email, password))
                    with open('Created [CAPTURE].txt', 'a', encoding='UTF-8') as f:
                        f.write('%s:%s | Username: %s | Gender: %s | Date of Birth: %s/%s-%s\n' % (
                            email, password, username, gender.replace(gender[0], gender[0].upper()), birth_day,
                            birth_month,
                            birth_year))
